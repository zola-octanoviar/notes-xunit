...
namespace notes.Models
{
    public class NoteController : Controller
    {
        ...
        private IOptionsSnapshot<NoteSettings> _settings;
 
        public NoteController(INoteService note, IOptionsSnapshot<NoteSettings> settings)
        {
            _note = note;
            _settings = settings;
        }
        ...
        {
            var posts = await _note.GetPosts(_settings.Value.PostsPerPage, _settings.Value.PostsPerPage * page);
            ViewData["Title"] = _settings.Value.Name + " - Notes on ASP.NET Core";
            ViewData["Description"] = _settings.Value.Description;
            ViewData["prev"] = $"/{page + 1}/";
            ViewData["next"] = $"/{(page <= 1 ? null : page - 1 + "/")}";
            return View("Views/Note/Index.cshtml", posts);
        }

        [Route("/note/category/{category}/{page:int?}")]
        [OutputCache(Profile = "default")]
        public async Task<IActionResult> Category(string category, int page = 0)
        {
            var posts = (await _note.GetPostsByCategory(category)).Skip(_settings.Value.PostsPerPage * page).Take(_settings.Value.PostsPerPage);
            ViewData["Title"] = _settings.Value.Name + " " + category;
            ViewData["Description"] = $"Articles posted in the {category} category";
            ViewData["prev"] = $"/note/category/{category}/{page + 1}/";
            ViewData["next"] = $"/note/category/{category}/{(page <= 1 ? null : page - 1 + "/")}";
            return View("Views/Note/Index.cshtml", posts);
        }
        ...
        {
            return LocalRedirectPermanent($"/note/{slug}");
        }
        ...
        {
            var post = await _note.GetPostBySlug(slug);
        ...
        public async Task<IActionResult> Edit(string id)
        {
            ...
            var post = await _note.GetPostById(id);
        ...
        public async Task<IActionResult> UpdatePost(Post post)
        {
            ...
            var existing = await _note.GetPostById(post.ID) ?? post;
            string categories = Request.Form["categories"];
            string title = (post.Title.Trim() != null ) ? post.Title.Trim() : "need title for this post";

            existing.Categories = categories.Split(",", StringSplitOptions.RemoveEmptyEntries).Select(c => c.Trim().ToLowerInvariant()).ToList();
            existing.Title = title;
            existing.Slug = Models.Post.CreateSlug(post.Title);
            existing.IsPublished = post.IsPublished;
            existing.Content = post.Content.Trim();
            string content = post.Content.Trim();
            existing.Excerpt = content.Substring(0, 500);;

            await _note.SavePost(existing);
            ...
            foreach (Match match in imgRegex.Matches(post.Content))
            {
                ...
                if (srcNode != null && fileNameNode != null)
                {
                    ...
                    if (base64Match.Success)
                    {
                        ...
                        srcNode.Value = await _note.SaveFile(bytes, fileNameNode.Value).ConfigureAwait(false);
        ...
        [Route("/note/deletepost/{id}")]
        [HttpPost, Authorize, AutoValidateAntiforgeryToken]
        public async Task<IActionResult> DeletePost(string id)
        {
            var existing = await _note.GetPostById(id);

            if (existing != null)
            {
                await _note.DeletePost(existing);
                return Redirect("/");
            }
        ...
        public async Task<IActionResult> AddComment(string postId, Comment comment)
        {
            ...
            if (!Request.Form.ContainsKey("website"))
            {
                ...
                await _note.SavePost(post);
            }

            return Redirect(post.GetLink() + "#" + comment.ID);
        }

        [Route("/note/comment/{postId}/{commentId}")]
        [Authorize]
