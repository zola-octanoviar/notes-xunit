...
namespace notes
{
    public class MetaWeblogService : IMetaWeblogProvider
    {
        ...
        public MetaWeblogService(INoteService note, IConfiguration config, IHttpContextAccessor context)
        {
            _note = note;
            _config = config;
            _context = context;
        ...
        public string AddPost(string noteid, string username, string password, WilderMinds.MetaWeblog.Post post, bool publish)
        {
            ...
            _note.SavePost(newPost).GetAwaiter().GetResult();
        ...
        public bool DeletePost(string key, string postid, string username, string password, bool publish)
        {
            ...
            var post = _note.GetPostById(postid).GetAwaiter().GetResult();

            if (post != null)
            {
                _note.DeletePost(post).GetAwaiter().GetResult();
                return true;
            }
        ...
        public bool EditPost(string postid, string username, string password, WilderMinds.MetaWeblog.Post post, bool publish)
        {
            ...
            var existing = _note.GetPostById(postid).GetAwaiter().GetResult();
            ...
            if (existing != null)
            {
                ...
                _note.SavePost(existing).GetAwaiter().GetResult();
        ...
        public CategoryInfo[] GetCategories(string noteid, string username, string password)
        {
            ValidateUser(username, password);

            return _note.GetCategories().GetAwaiter().GetResult()
                           .Select(cat =>
                               new CategoryInfo
                               {
                                   categoryid = cat,
        ...
        public WilderMinds.MetaWeblog.Post[] GetRecentPosts(string noteid, string username, string password, int numberOfPosts)
        {
            ValidateUser(username, password);

            return _note.GetPosts(numberOfPosts).GetAwaiter().GetResult().Select(p => ToMetaWebLogPost(p)).ToArray();
        }
        ...
        public MediaObjectInfo NewMediaObject(string noteid, string username, string password, MediaObject mediaObject)
        {
            ValidateUser(username, password);
            byte[] bytes = Convert.FromBase64String(mediaObject.bits);
            string path = _note.SaveFile(bytes, mediaObject.name).GetAwaiter().GetResult();
